...
if ($color != '') {
	$this->counters_atts['style_counters'] .= '#cmsmasters_counter_' . $unique_id . '.cmsmasters_counter:not(.counter_has_icon) .cmsmasters_counter_counter_wrap,' . 
	'#cmsmasters_counter_' . $unique_id . '.cmsmasters_counter.counter_has_icon .cmsmasters_counter_inner:before { ' . 
	...
	'" data-percent="' . $value . '">' . "\n" . 
		...
		'">' . "\n" . 
			'<span class="cmsmasters_counter_counter_wrap">' . "\n";
				if ($value_prefix !='') {
					$out .= '<span class="cmsmasters_counter_prefix">' . 
						esc_html($value_prefix) . 
					'</span>';
				}
				
				
				$out .= '<span class="cmsmasters_counter_counter">0</span>';
				
				
				if ($value_suffix !='') {
					$out .= '<span class="cmsmasters_counter_suffix">' . 
						esc_html($value_suffix) . 
					'</span>';
				}
			$out .= '</span>' . "\n" . 
